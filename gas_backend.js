...
/**
 * 管理用パスワード
 * ★★★ 必ずここを変更してからGASにデプロイしてください ★★★
 * 推奨: 英数字記号を混ぜた16文字以上のランダムな文字列
 * 例: "Lv@8pXq2#mR5kZ9w"
 */
var ADMIN_PASSWORD = "★ここを自分のパスワードに変更してください★";
...
function doGet(e) {
  try {
    ...
    // セキュリティ強化: GETリクエストでは管理者認証を行わない
    // 管理者データの取得はPOSTリクエスト(type='admin_get')のみで行う
    var isAuth = false;
...
function doPost(e) {
  try {
    ...
    // 0. 管理者用データ取得 (GETではなくPOSTで認証する)
    if (type === 'admin_get') {
      if (params.password !== ADMIN_PASSWORD) {
        return createJsonResponse({ "result": "error", "message": "Unauthorized" });
      }
      var eventSheet2 = getOrCreateSheet(ss, "Events", ["id", "date", "time", "type", "title", "description", "imageUrl", "capacity"]);
      var reservationSheet2 = getOrCreateSheet(ss, "Reservations", ["submittedAt", "type", "target", "name", "email", "count", "message", "paid"]);
      return createJsonResponse({
        auth: true,
        events: getRowsAsJson(eventSheet2),
        reservations: getRowsAsJson(reservationSheet2)
      });
    }
    ...
    // 3. オーディションエントリー
    if (type === 'audition') {
      var audSheet = getOrCreateSheet(ss, "Auditions", ["submittedAt", "name", "email", "age", "musicUrl", "message"]);
      audSheet.appendRow([
        new Date(),
        params.name || '',
        params.email || '',
      ...
      if (params.email) {
        try {
          MailApp.sendEmail({
            to: params.email,
            subject: "【lala Audition】エントリーを受け付けました",
            body: params.name + " 様\n\nlala ボーカルオーディションへのご応募、誠にありがとうございます。\n\nエントリーを受け付けました。\n2〜3営業日以内に、事務局よりご連絡いたします。\n\n--\nlala Official",
            name: "lala Official",
            replyTo: "livebarvisca@anthemys.jp"
          });
        } catch(mailErr) {
          console.log("Audition email error: " + mailErr.message);
        }
      }
      return createJsonResponse({ "result": "success" });
    }
...
function sendConfirmationEmail(type, name, email, count, message, resDate, resTime, liveTitle, resType, duration) {
  ...
  if (type === 'bar' && resType === 'studio') {
    ...
    body += "この度は、lala 貸しスタジオ をご予約いただき誠にありがとうございます。\n"
         ...
         + "※決済リンクはgas_backend.jsのsendConfirmationEmail関数内に設定してください\n\n"
         + "ご利用を心よりお待ち申し上げております。\n\n";
  } else if (type === 'bar') {
